...
namespace Cici.Reporter
{
    ...
    public class ConsoleReporter : IReporter
    {
        ...
        {
            List<FlakyTestResult> flakyList = flakyTests.ToList();
            if (!flakyList.Any())
            {
                return;
            }
            ...
            int index = 1;
            foreach (FlakyTestResult? test in flakyList.OrderBy(t => t.PassRate))
            {
                Console.Write($"  {index}. ");
                Console.ForegroundColor = ConsoleColor.Cyan;
        ...
        {
            List<FlakyTestResult> stableList = stableTests.Take(5).ToList();
            if (!stableList.Any())
            {
                return;
            }
            ...
            foreach (FlakyTestResult test in stableList)
            {
                Console.Write("  • ");
        ...
        {
            List<FlakyTestResult> failingList = failingTests.ToList();
            if (!failingList.Any())
            {
                return;
            }
            ...
            foreach (FlakyTestResult? test in failingList.Take(5))
            {
                Console.Write("  • ");
                ...
                string? commonError = test.ExecutionResults
        ...
        private void PrintErrorPatterns(Dictionary<string, int> errorPatterns)
        {
            ...
            foreach (KeyValuePair<string, int> pattern in errorPatterns.Take(5))
            {
                Console.Write("  • ");
        ...
        private string TruncateTestName(string name, int maxLength)
        {
            ...
            if (parts.Length > 2)
            {
                string shortened = $"...{parts[^2]}.{parts[^1]}";
            ...
            return "..." + name[(name.Length - maxLength + 3)..];
        }
